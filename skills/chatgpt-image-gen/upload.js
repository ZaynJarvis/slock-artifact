...
//
//   node upload.js ./images/001.png [--base-url https://image.zaynjarvis.com] [--key-env ZAYN_IMAGE_KEY]

const fs = require('fs');
const path = require('path');

function parseArgs(argv) {
  const args = { baseUrl: 'https://image.zaynjarvis.com', keyEnv: 'ZAYN_IMAGE_KEY' };
  const positional = [];
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === '--base-url') args.baseUrl = argv[++i];
      ...
      console.log('Usage: node upload.js <file> [--base-url URL] [--key-env NAME]');
      process.exit(0);
